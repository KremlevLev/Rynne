...
Логи установленного desktop-приложения находятся в
`%LOCALAPPDATA%\ai.nova.desktop\logs`. Папку можно открыть через
**Настройки → Диагностика → Открыть папку логов**. `rynne-core.log` содержит
общий журнал Core, `rynne-desktop.log` — события desktop-supervisor, а
`sessions\rynne-core-*.log` — отдельный неизменяемый лог каждого запуска.
Для полного безопасного отчёта запустите `scripts\collect-diagnostics.ps1`:
скрипт создаст ZIP на рабочем столе и не добавит туда `.env` или API-ключи.
