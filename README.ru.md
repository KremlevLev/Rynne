...
[![Tests](https://img.shields.io/badge/tests-740%20passing-22C55E?style=for-the-badge)](#проверка)
...
- Nova автоматически замечает многосоставные инженерные и исследовательские задачи.
...
- Reviewer собирает выводы без подмены исходной цели, а реальные инструменты,
  permissions и проверка результата остаются у главной Nova.
...
восьми частей: быстрый старт, инструменты и task ledger, голос, «Nova рядом», skills,
...
`x-slow/slow/normal/fast/x-fast`. Nova больше не ускоряет уже готовую запись, поэтому
...
Nova будет получать разрешённые новые сообщения, хранить локальный кэш и после
...
В Nova теперь есть опциональный локальный Telegram MCP для обычного аккаунта,
...
перед отправкой Nova следует выбранному режиму разрешений: без запроса, только для рискованных действий или перед каждым инструментом. Для визуального
...
Версии Nova, содержащие текущий файл [`LICENSE`](LICENSE), распространяются по **Functional Source License 1.1, Apache 2.0 Future License** (`FSL-1.1-ALv2`). Разрешены личное и внутреннее использование, обучение, исследования, модификация и распространение в допустимых целях. Предоставлять Nova или практически аналогичную функциональность как конкурирующий коммерческий продукт или сервис условия FSL не разрешают.
