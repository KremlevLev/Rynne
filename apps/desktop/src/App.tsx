...
function agentProgressCopy(payload: JsonObject, locale: UiLocale): {
  ...
  const copies: Record<string, [string, string, string, string]> = {
    ...
    telegram_resolving: ["Ищу получателя в Telegram", "Resolving Telegram recipient", "Сопоставляю имя с доступными чатами напрямую, без обращения к модели.", "Matching the name against available chats directly, without a model call."],
    telegram_sending: ["Отправляю сообщение в Telegram", "Sending the Telegram message", "Получатель найден. Передаю сообщение через Telegram MCP.", "Recipient found. Sending through Telegram MCP."],
    telegram_forwarding: ["Пересылаю сообщение в Telegram", "Forwarding the Telegram message", "Сопоставляю оба чата, нахожу исходное сообщение и выполняю настоящую пересылку.", "Resolving both chats, locating the original message, and performing a real forward."],
