# Задача для агента: новый фронтенд Nova

Нужно сделать **очень красивый, премиальный и живой desktop UI** для Nova — голосового Windows-агента.
...
Nova должна выглядеть как дорогой, быстрый и умный персональный агент, а не как обычный чат-бот.
...
6. Не создавай «фейковый UI». Все статусы, tool calls, прогресс и результаты должны быть привязаны к реальным данным Nova.
...
- Градиенты использовать тонко: в фоне, активной орбите Nova, подсветке focus.
...
Сообщение Nova должно быть не обычным полотном текста, а **умной структурой**:
...
- Текст Nova появляется постепенно, но без неприятного «печатания по одной букве».
...
# 7. Nova Orb и голосовой интерфейс

Сделай **узнаваемую визуальную сущность Nova**: orb / ядро / световая форма.
...
Когда Nova работает, пользователь должен видеть, что происходит, но не утонуть в технических деталях.
...
Nova создаёт файлы, заметки, код, отчёты и исследования. Их нужно показывать красиво.
...
Проверить конфликты с Windows и текущими hotkeys Nova.
